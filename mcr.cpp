...
int main(){
    ...
    for (int n=0; n<9; n++){
        ...
        if(game[i][j] != 'X' && game[i][j] != 'O' && i <= 2 && i >= 0 && j <= 2 && j >= 0) { //check if the input is appropriate.
            goto next; //if the input is appropriate, go next
        }
        else{
            cout << "You can not mark there." << endl;
            while ((c = getchar()) != '\n');
            goto rewrite; //if the input is not apropriate, ask for input again.
        }
